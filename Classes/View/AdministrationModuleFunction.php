...
class AdministrationModuleFunction extends \TYPO3\CMS\Backend\Module\AbstractFunctionModule
{
    ...
    protected function addModuleStyles()
    {
        $this->pObj->doc->inDocStyles .= '
			...
			TABLE#tx-hellurl-pathcacheTable TD { vertical-align: top; }
		...
		' . \Nimut\Hellurl\ViewHelpers\PageBrowserViewHelper::getInlineStyles();
    }
    ...
    protected function initializeTree()
    {
        ...
        $tree->addField('tx_hellurl_pathsegment', true);
    ...
    public function renderModule(\TYPO3\CMS\Backend\Tree\View\PageTreeView $tree)
    {
        ...
        $output = '
		<table border="0" cellspacing="1" cellpadding="0" id="tx-hellurl-pathcacheTable" class="lrPadding c-list">' . $output . '
		</table>';
    ...
    {
        $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_hellurl_pathcache', 'cache_id=' . intval($cache_id));
    }
    ...
    public function copyPathCacheEntry(&$oEntry)
    {
        ...
        $GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_hellurl_pathcache', $cEntry);
    ...
    public function editPathCacheEntry($cache_id, $value)
    {
        ...
        list($page_id_rec) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('page_id', 'tx_hellurl_pathcache', 'cache_id=' . intval($cache_id));
        ...
        foreach ($tree->tree as $row) {
            ...
            if (!count($displayRows) || $subcmd === 'displayed') {
                ...
                foreach ($displayRows as $c => $inf) {
                    ...
                    if (count($GLOBALS['TYPO3_DB']->exec_SELECTgetRows('url_hash', 'tx_hellurl_redirects', 'url_hash=' . intval(\TYPO3\CMS\Core\Utility\GeneralUtility::md5int($inf['content']))))) {
    ...
    public function uniqueAlias()
    {
        ...
        if ($tableName && isset($overviewRows[$tableName])) {    // Show listing of single table:
            ...
            if ($cmd === 'delete') {
                if ($entry === 'ALL') {
                    $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_hellurl_uniqalias', 'tablename=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($tableName, 'tx_hellurl_uniqalias'));
                } else {
                    $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_hellurl_uniqalias', 'tablename=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($tableName, 'tx_hellurl_uniqalias') . ' AND uid=' . intval($entry));
                }
            }
            if ($cmd === 'flushExpired') {
                $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_hellurl_uniqalias', 'tablename=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($tableName, 'tx_hellurl_uniqalias') . ' AND expire>0 AND expire<' . intval(time()));
            }

            // Select rows:
            $tableContent = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
                '*',
                'tx_hellurl_uniqalias',
    ...
    public function configView()
    {
        ...
        $theVar = $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['hellurl'];
        $tree = $arrayBrowser->tree($theVar, '', '');

        $tree = '<hr/>
		<b>$GLOBALS[\'TYPO3_CONF_VARS\'][\'EXTCONF\'][\'hellurl\']</b>
		<br/>
    ...
    {
        foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['hellurl'] as $host => $config) {
            if ($host != '_DEFAULT') {
                $hostName = $host;
                while ($config !== false && !is_array($config)) {
                    $host = $config;
                    $config = (isset($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['hellurl'][$host]) ? $GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['hellurl'][$host] : false);
                }
    ...
    protected function getRedirectsTableContent($sortingParameter, $sortingDirection)
    {
        ...
        if ($seachPath) {
            $seachPathDecoded = $GLOBALS['TYPO3_DB']->quoteStr(
                $GLOBALS['TYPO3_DB']->escapeStrForLike(rawurlencode($seachPath), 'tx_hellurl_redirects'),
                'tx_hellurl_redirects');
            $seachPath = $GLOBALS['TYPO3_DB']->quoteStr(
                $GLOBALS['TYPO3_DB']->escapeStrForLike($seachPath, 'tx_hellurl_redirects'),
                'tx_hellurl_redirects');
        ...
        if ($sortingParameter !== 'domain_limit') {
            $query = 'SELECT t1.* FROM tx_hellurl_redirects t1' . ($condition ? ' WHERE ' . $condition : '') .
                ' ORDER BY ' . $sortingParameter . ' ' . $sortingDirection .
                ' LIMIT ' . $start . ',' . $resultsPerPage;
        } else {
            $query = 'SELECT t1.* FROM tx_hellurl_redirects t1' .
        ...
        list($count) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
            'COUNT(*) AS t', 'tx_hellurl_redirects', $condition);
        $totalResults = $count['t'];
        if ($totalResults > $resultsPerPage) {
            $pageBrowser = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance('Nimut\\Hellurl\\ViewHelpers\\PageBrowserViewHelper');
            /** @var \Nimut\Hellurl\ViewHelpers\PageBrowserViewHelper $pageBrowser */
    ...
    {
        $tsConfig = \TYPO3\CMS\Backend\Utility\BackendUtility::getModTSconfig($this->pObj->id, 'tx_hellurl.' . $view . '.pagebrowser.resultsPerPage');
        $resultsPerPage = $tsConfig['value'];
        return \TYPO3\CMS\Core\Utility\MathUtility::canBeInterpretedAsInteger($resultsPerPage) ? intval($resultsPerPage) : \Nimut\Hellurl\ViewHelpers\PageBrowserViewHelper::RESULTS_PER_PAGE_DEFAULT;
    }
    ...
    protected function generateSingleRedirectContent(array $rec, $page)
    {
        ...
        // Error:
        $errorMessage = '';
        $pagesWithURL = array_keys($GLOBALS['TYPO3_DB']->exec_SELECTgetRows('page_id', 'tx_hellurl_urlencodecache', 'content=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($rec['url'], 'tx_hellurl_urlencodecache'), '', '', '', '', 'page_id'));
    ...
    protected function getRedirectViewHeader($sortingDirection)
    {
        ...
        return '<table border="0" cellspacing="2" cellpadding="2" id="tx-hellurl-pathcacheTable" class="lrPadding c-list">' .
            '<tr class="bgColor5 tableheader">' .
            '<td>&nbsp;</td>' .
    ...
    protected function deleteRedirectEntry()
    {
        ...
        if ($uid) {
            $GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_hellurl_redirects',
                'uid=' . intval($uid)
            );
        }
    }
    ...
    protected function getNewButton()
    {
        $content = '<div style="margin:0 0 0.5em 3px"><a href="' . $this->linkSelf('&cmd=new') . '">' .
            $this->getIcon('gfx/new_el.gif', 'width="11" height="12"', $GLOBALS['BACK_PATH'], 'New entry') .
    ...
    protected function getRedirectEditForm()
    {
        ...
        list($row) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows(
            'url,url_hash,destination,has_moved,domain_limit', 'tx_hellurl_redirects',
            'uid=' . intval($uid));
        if (is_array($row)) {
            $page = max(1, intval(\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('page')));
        ...
        if (\TYPO3\CMS\Core\Utility\GeneralUtility::_GP('_edit_save')) {
            ...
            foreach ($data as $fields) {
                ...
                list($row) = $GLOBALS['TYPO3_DB']->exec_SELECTgetRows('COUNT(*) AS t',
                    'tx_hellurl_redirects',
                    'url=' . $GLOBALS['TYPO3_DB']->fullQuoteStr($fields['source'], 'tx_hellurl_redirects') .
            ...
            foreach ($databaseInsertData as $data) {
                $GLOBALS['TYPO3_DB']->exec_INSERTquery('tx_hellurl_redirects', $data);
            }
